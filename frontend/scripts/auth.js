...
class AuthManager {
    ...
    async signUp(email, password, userData) {
        if (this.useFirebase) {
            try {
                ...
                // NOTE: Do NOT automatically log in the user - let them log in manually
                return { success: true, user: result.user };
            } catch (error) {
                return { success: false, error: this.getErrorMessage(error.code) };
            }
            ...
            try {
                ...
                // NOTE: Do NOT automatically log in the user - return success and let them log in manually
                return { success: true, user: { uid: newUser.uid, email: newUser.email } };
            } catch (error) {
    ...
    async onLogin(user) {
        ...
        // Use window.app to ensure we're accessing the global instance
        if (typeof window.app === 'undefined' || !window.app) {
            console.warn('app object not yet initialized, storing user data for later');
            // Store user data in session for app to pick up
        ...
        } else if (typeof window.app !== 'undefined' && window.app) {
